...
class TxPool {
    ...
    async _validateTransaction(tx, senderAddress, senderNonce) {
        ...
        const gasPrice = new ethereumjs_util_1.BN(tx.gasPrice);
    ...
    _replaceTx(txs, newTx) {
        ...
        const currentGasPrice = new ethereumjs_util_1.BN(deserializedExistingTx.data.gasPrice);
        const newGasPrice = new ethereumjs_util_1.BN(newTx.data.gasPrice);
        const minNewGasPrice = this._getMinNewGasPrice(currentGasPrice);
        if (newGasPrice.lt(minNewGasPrice)) {
            throw new errors_1.InvalidInputError(`Replacement transaction underpriced. A gas price of at least ${minNewGasPrice.toString()} is necessary to replace the existing transaction.`);
        }
    ...
    _getMinNewGasPrice(currentGasPrice) {
