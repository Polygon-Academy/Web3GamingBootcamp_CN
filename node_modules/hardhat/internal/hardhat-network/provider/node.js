...
class HardhatNode extends events_1.default {
    constructor(_vm, _stateManager, _blockchain, _txPool, _automine, _minGasPrice, _blockTimeOffsetSeconds = new ethereumjs_util_1.BN(0), genesisAccounts, tracingConfig, _forkNetworkId) {
        super();
        this._vm = _vm;
    ...
    static async create(config) {
        ...
        const node = new HardhatNode(vm, stateManager, blockchain, txPool, automine, minGasPrice, initialBlockTimeOffset, genesisAccounts, tracingConfig, forkNetworkId);
        return [common, node];
    }
...
Hardhat Network's forking functionality only works with blocks from at least spuriousDragon.`);
        ...
        return this._runInBlockContext(new ethereumjs_util_1.BN(block.header.number).subn(1), async () => {
            ...
            for (const tx of block.transactions) {
                ...
                if (tx.type === 0) {
                    ...
                    throw new errors_2.InternalError("Only legacy and EIP2930 txs are supported");
                }
    ...
    async _validateAutominedTx(tx) {
        ...
        // validate gas price
        const txGasPrice = new ethereumjs_util_1.BN(tx.gasPrice); // TODO remove this "as any"
        if (txGasPrice.lt(this._minGasPrice)) {
            throw new errors_2.InvalidInputError(`Transaction gas price is ${txGasPrice}, which is below the minimum of ${this._minGasPrice}`);
        }
    }
    /**
    ...
    _isTxMinable(tx) {
        const txGasPrice = new ethereumjs_util_1.BN(tx.gasPrice);
